...
void BleComboKeyboard::setServer(BLEServer *pServer) {
...
void BleComboKeyboard::taskServer(void *pvParameter) {
  ...
  bleKeyboardInstance->connectionStatus->hid = bleKeyboardInstance->hid;
  ...
  std::cout << "Service UUID: " << bleKeyboardInstance->hid->hidService()->getUUID().toString() << "\n";
  std::cout << "Input Keyboard Characteristic UUID: " << bleKeyboardInstance->inputKeyboard->getUUID().toString() << "\n";
  std::cout << "Output Keyboard Characteristic UUID: " << bleKeyboardInstance->outputKeyboard->getUUID().toString() << "\n";
  std::cout << "Mouse Characteristic UUID: " << bleKeyboardInstance->inputMouse->getUUID().toString() << "\n";
  ...
  ///////
  // BLEService *pService = bleKeyboardInstance->pServer->createService(SERVICE_UUID);
  ...
  // MyCallbacks *rxCallback = new MyCallbacks();
...
void BleComboKeyboard::sendReport(KeyReport *keys) {
  if (this->isConnected()) {
    ...
    this->inputKeyboard->notify(true, 1);
  }
}
...
void BleComboKeyboard::sendReport(MediaKeyReport *keys) {
  if (this->isConnected()) {
    ...
    this->inputMediaKeys->notify(true, 1);
  }
}
